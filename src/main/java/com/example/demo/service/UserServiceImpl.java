...
public class UserServiceImpl implements UserService {
    private final UserRepository userRepository;  // 사용자 저장소 (JPA Repository)
     ...
     * @throws UserAlreadyExistsException 동일한 이메일이 이미 존재하는 경우 발생
     */
    @Override
    ...
    public UserDTO save(SignUpRequest signUpRequest) {
        ...
        if (this.userRepository.existsByEmail(signUpRequest.getEmail())) {
            throw new UserAlreadyExistsException("이미 존재하는 이메일입니다: " + signUpRequest.getEmail());
        }
        ...
        User savedUser = this.userRepository.save(
                User.builder()
                        ...
                        .roles(signUpRequest.getRoles()) // 사용자 역할(Role) 설정
    ...
    public Optional<User> authenticate(String email, String rawPassword) {
        return this.userRepository.findByEmail(email)
                .filter(user -> passwordEncoder.matches(rawPassword, user.getPassword())); // 비밀번호 검증
    }
}
